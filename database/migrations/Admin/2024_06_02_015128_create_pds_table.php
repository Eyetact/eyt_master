...
return new class extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create('pds', function (Blueprint $table) {
            ...
            if (!Schema::hasColumn('pds', 'customer_group_id')){
                $table->integer('customer_group_id')->nullable();
            }
            if (!Schema::hasColumn('pds', 'customer_id')){
                $table->integer('customer_id')->nullable();
            }

            if (!Schema::hasColumn('pds', 'assign_id')){
                $table->integer('assign_id')->nullable();
            }

              if (!Schema::hasColumn('pds', 'global')){
                $table->boolean('global')->default(0);
            }
               if (!Schema::hasColumn('pds', 'status')){
