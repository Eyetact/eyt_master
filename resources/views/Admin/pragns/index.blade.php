...
@section('page-header')
    ...
    <div class="page-header">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">{{ __('Pragnas') }}</h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#"><i class="fe fe-layout mr-2 fs-14"></i>Settings</a></li>
                <li class="breadcrumb-item active" aria-current="page"><a href="#">{{ __('Pragnas') }}</a></li>
            </ol>
        </div>
        <div class="page-rightheader">
        @php
                    $model_id = App\Models\Permission::where('name', 'create.pragna')
                    ->orWhere('name', 'create.pragn')?->first()?->module;
                    $model = App\Models\Module::find($model_id)
                    // dd($model_id);
                @endphp
        @canany(['create.pragn','create.pragna'])
...
@endif
            <div class="card">
                ...
                <div class="card-title">{{ __('Pragnas') }}</div>
                        ...
                        <div class="col-12">
                            @php
                                $module = \App\Models\Module::where(
                                    'code',
                                    App\Generators\GeneratorUtils::singularSnakeCase('pragn'),
                                )
                                    ->orWhere('code', App\Generators\GeneratorUtils::pluralSnakeCase('pragn'))
                                    ->orWhere('code', 'pragn')
                                    ?->first();
                            @endphp
                            <div class="row">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form  action="{{route('assign-record', 'pragn')}}" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-sm-10">
                            <div class="input-box" style="margin:0 !important">
                                <select class="google-input assign_g" name="ids[]" tabindex="null" multiple>
                                    @foreach (\App\Models\Admin\Pragn::where('user_id',1)->where('global',1)->where('status','active')->where('assign_id',null)
                                    ->orWhere('user_id',auth()->user()->user_id)->where('global',1)->where('status','active')->where('assign_id',null)->get() as $item)
                                    @if($item)
                                    @php
                                        $column = \App\Generators\GeneratorUtils::getColumnAfterId('pragn');
                                    @endphp
...
@section('js')
    ...
    function changeEvents() {
        ...
        @php
            $model = \App\Models\Module::where('code','pragn')->first();
        @endphp
    ...
    @include('admin.pragns.include.multi')
    @include('admin.pragns.include.custom')
@endsection
