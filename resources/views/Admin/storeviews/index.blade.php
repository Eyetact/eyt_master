...
@section('page-header')
    ...
    <div class="page-header">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">{{ __('Store Views') }}</h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#"><i class="fe fe-layout mr-2 fs-14"></i>Settings</a></li>
                <li class="breadcrumb-item active" aria-current="page"><a href="#">{{ __('Store Views') }}</a></li>
            </ol>
        </div>
        <div class="page-rightheader">
        @php
                    $model_id = App\Models\Permission::where('name', 'create.store view')
                    ->orWhere('name', 'create.storeview')?->first()?->module;
                    $model = App\Models\Module::find($model_id)
                    // dd($model_id);
                @endphp
        @canany(['create.storeview','create.store view'])
...
@endif
            <div class="card">
                ...
                <div class="card-title">{{ __('Store Views') }}</div>
                        ...
                        <div class="col-12">
                            @php
                                $module = \App\Models\Module::where(
                                    'code',
                                    App\Generators\GeneratorUtils::singularSnakeCase('storeview'),
                                )
                                    ->orWhere('code', App\Generators\GeneratorUtils::pluralSnakeCase('storeview'))
                                    ->orWhere('code', 'storeview')
                                    ?->first();
                            @endphp
                            <div class="row">
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form  action="{{route('assign-record', 'storeview')}}" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-sm-10">
                            <div class="input-box" style="margin:0 !important">
                                <select class="google-input assign_g" name="ids[]" tabindex="null" multiple>
                                    @foreach (\App\Models\Admin\Storeview::where('user_id',1)->where('global',1)->where('status','active')->where('assign_id',null)
                                    ->orWhere('user_id',auth()->user()->user_id)->where('global',1)->where('status','active')->where('assign_id',null)->get() as $item)
                                    @if($item)
                                    @php
                                        $column = \App\Generators\GeneratorUtils::getColumnAfterId('storeview');
                                    @endphp
...
@section('js')
    ...
    function changeEvents() {
        ...
        @php
            $model = \App\Models\Module::where('code','storeview')->first();
        @endphp
    ...
    @include('admin.storeviews.include.multi')
    @include('admin.storeviews.include.custom')
@endsection
