...
@section('page-header')
    ...
    <div class="page-header">
        <div class="page-leftheader">
            <h4 class="page-title mb-0">{{ __('Testmodules') }}</h4>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="#"><i class="fe fe-layout mr-2 fs-14"></i>Settings</a></li>
                <li class="breadcrumb-item active" aria-current="page"><a href="#">{{ __('Testmodules') }}</a></li>
            </ol>
        </div>
        ...
        @php
                           ...
                           $model_id = \App\Models\Module::where(
                                    'code',
                                    App\Generators\GeneratorUtils::singularSnakeCase('testmodule'),
                                )
                                    ->orWhere('code', App\Generators\GeneratorUtils::pluralSnakeCase('testmodule'))
                                    ->orWhere('code', 'testmodule')
                                    ?->first()->id;

                    $model = App\Models\Module::find($model_id);
                    //dd($model_id);
                @endphp
        @canany(['create.testmodule','create.testmodule'])
...
@endif
            <div class="card">
                ...
                <div class="card-title">{{ __('Testmodules') }}</div>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form  action="{{route('assign-record', 'testmodule')}}" method="post">
                    @csrf
                    <div class="row">
                        <div class="col-sm-10">
                            <div class="input-box" style="margin:0 !important">
                                <select class="google-input assign_g" name="ids[]" tabindex="null" multiple>
                                    @foreach (\App\Models\Admin\Testmodule::where('user_id',1)->where('global',1)->where('status','active')->where('assign_id',null)
                                    ->orWhere('user_id',auth()->user()->user_id)->where('global',1)->where('status','active')->where('assign_id',null)->get() as $item)
                                    @if($item)
                                    @php
                                        $column = \App\Generators\GeneratorUtils::getColumnAfterId('testmodule');
                                    @endphp
...
@section('js')
    ...
    function changeEvents() {
        ...
        @php
            $model = \App\Models\Module::where('code','testmodule')->first();
        @endphp
    ...
    @include('admin.testmodules.include.multi')
    @include('admin.testmodules.include.custom')
@endsection
